  ...
  import { setIsCleanerOpen, setIsFontSizesOpen, setIsFontFamiliesOpen, setIsColorsOpen, setIsResizeCanvasOpen } from "./popupElementsSlice"

  export function PopupStatus(namePopup: string, dispatch: Dispatch<AnyAction>, isOpen: boolean) {
      switch(namePopup) {
      case "cleaner": {
        if(isOpen === false){
