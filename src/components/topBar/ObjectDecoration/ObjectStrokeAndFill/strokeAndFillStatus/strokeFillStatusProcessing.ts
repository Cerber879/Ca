...
=======
import { setActiveObjStrokeFill, setBtnStyleObjStrokeFill } from "./strokeFillSettings"
import { AnyAction, Dispatch } from "redux";

export function objStrokeFillStatusProcessing(nameProcess: string, dispatch: Dispatch<AnyAction>, activeObjStrokeFill: boolean, id: string) {
    switch(nameProcess) {
    case "objStrokeFillHover": {
        if (activeObjStrokeFill === false) { 
            dispatch(setBtnStyleObjStrokeFill({ backgroundColor: "#6489ef" }));
